...
# Reuses the exit_hook from rlm_mutator — it is LD_PRELOAD'd into the target,
# independent of which Python mutator is running. Copy/build a local one if
# you want rllm to ship without an rlm_mutator dependency.
EXIT_HOOK_SO="$HOME/Documents/AFLplusplus/custom_mutators/rlm_mutator/exit_hook.so"
RLLM_DIR="$HOME/Documents/AFLplusplus/custom_mutators/rllm"
...
require_dir "$RLLM_DIR" "rllm module directory"
...
# Legacy var names: the exit_hook writes target stderr to RLM_STDERR_FILE,
# and rllm.py reads from the same path. Rename together if changed.
